...
<br/>PHPlist 無法處理大於 1Mb 的檔案',
  'If you check "Test Output", you will get the list of parsed emails on screen, and the database will not be filled with the information. This is useful to find out whether the format of your file is correct. It will only show the first 50 records.' => '如果您勾選 "測試輸出"，您會看到解析後的電子郵件列表，資訊並不會儲存到資料庫中；這是用來確認檔案格式正確與否，只會顯示前 50 筆資料。',
  'If you check "Show Warnings", you will get warnings for invalid records. Warnings will only be shown if you check "Test Output". They will be ignored when actually importing. ' => '如果您勾選 "顯示警告"，您將會看到個別資料的警告訊息；警告訊息只會出現在 "測試輸出"時，實際匯入時會被忽略。',
  'If you check "Omit Invalid", invalid records will not be added. Invalid records are records without an email. Any other attributes will be added automatically, ie if the country of a record is not found, it will be added to the list of countries.' => '如果您勾選 "忽略錯誤資料"，錯誤的資料將不會新增；錯誤的資料指的是沒有包含電子郵件的項目。 其他的屬性會自動加入，例如如果找到一筆資料的國家欄位不存在，它會自動被新增到系統的國家清單中。',
  'Assign Invalid will be used to create an email for subscribers with an invalid email address. You can use values between [ and ] to make up a value for the email. For example if your import file contains a column "First Name" and one called "Last Name", you can use "[first name] [last name]" to construct a new value for the email for this subscriber containing their first name and last name. The value [number] can be used to insert the sequence number for importing.' => '指定錯誤的功能是用來在使用者電子郵件格式錯誤時自動產生一個信箱，您可以使用 [ 與 ] 之間的數值來設定電子郵件。例如匯入的資料包含欄位像是 "First Name" 與 "Last Name"，您可以使用 "[first name] [last name]" 把這兩個欄位當作電子郵件的資料；而 [number] 的數值則用來插入匯入資料的流水號。',
  'If you check "Overwrite Existing", information about a subscriber in the database will be replaced by the imported information. Subscribers are matched by email or foreign key.' => '如果您勾選 "覆蓋現有資料"，存在資料庫的使用者資訊會被匯入的資料所取代，使用者比對的方式是透過電子郵件或外部鍵值。',
  'If you check "Retain Old Email", a conflict of two emails being the same will keep the old one and add "duplicate" to the new one. If you don&quot;t check it, the old one will get "duplicate" and the new one will take precedence.' => '如果您勾選 "保留舊的使用者信箱"，當兩個電子郵件因為重複而產生衝突時，舊的資料會被保留，並且建立一個重複的項目來儲存新資料。如果您沒有勾選，舊的資料將會被視為重複項目，而優先使用新的資料。',
  'If you choose "send notification email" the subscribers you are adding will be sent the request for confirmation of subscription to which they will have to reply. This is recommended, because it will identify invalid emails.' => '如果您選擇 "寄送通知郵件" ，被新增的使用者會收到訂閱的確認訊息，讓使用者能夠自行決定是否訂閱。建議您使用這個功能，因為匯入的資料可能包含大量錯誤的信箱。',
  'phplist Import  Results' => '匯入結果',
