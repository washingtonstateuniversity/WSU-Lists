...
'automatically added' => 'automatisk tilf&oslash;jet',
 'The file you upload will need to have the attributes of the records on    the first line.     Make sure that the email column is called "email" and not something like "e-mail" or     "Email Address".     Case is not important.          If you have a column called "Foreign Key", this will be used for synchronisation between an     external database and the phpList database. The foreignkey will take precedence when matching     an existing subscriber. This will slow down the import process. If you use this, it is allowed to have     records without email, but an "Invalid Email" will be created instead. You can then do     a search on "invalid email" to find those records. Maximum size of a foreign key is 100.          Warning: the file needs to be plain text. Do not upload binary files like a Word Document.     ' => '<p class="information">Filen du uploader skal have records attributterne p&aring; den f&oslash;rste linje.
    ...
    <br/>',
'The following limits are set by your server: Maximum size of a total data sent to server: %s Maximum size of each individual file: %s' => 'F&oslash;lgende begr&aelig;nsninger er sat af din server:<br/>
...
'If you check "Test Output", you will get the list of parsed emails on screen, and the database will not be filled with the information. This is useful to find out whether the format of your file is correct. It will only show the first 50 records.' => 'Hvis du har valgt "Test visning", vil du se den parsede liste med emails p&aring; sk&aelig;rmen, og databasen vil ikke modtage informationerne. Det er brugbart til at finde ud af om filens format er korrekt eller ej. Der vil kun blive vist de f&oslash;rste 50 records.',
'If you check "Show Warnings", you will get warnings for invalid records. Warnings will only be shown if you check "Test Output". They will be ignored when actually importing. ' => 'Hvis du v&aelig;lger "Vis advarsler", vil du f&aring; advarsler om ugyldige records. Advarsler vil kun blive vist hvis du v&aelig;lger "Test visning". De vil blive ignoreret under den rigtige import. ',
'If you check "Omit Invalid", invalid records will not be added. Invalid records are records without an email. Any other attributes will be added automatically, ie if the country of a record is not found, it will be added to the list of countries.' => 'Hvis du v&aelig;lger "Afvis ugyldig", vil ugyldige records ikke blive tilf&oslash;jet. Ugyldige records er records uden en email. Alle andre attributter vil blive tilf&oslash;jet automatisk, som fx hvis en records land ikke bliver fundet, s&aring; vil det blive tilf&oslash;jet til landelisten.',
'Assign Invalid will be used to create an email for subscribers with an invalid email address. You can use values between [ and ] to make up a value for the email. For example if your import file contains a column "First Name" and one called "Last Name", you can use "[first name] [last name]" to construct a new value for the email for this subscriber containing their first name and last name. The value [number] can be used to insert the sequence number for importing.' => '"Tilf&oslash;j ugyldig" vil oprette en email til brugere med en ugyldig email adresse.
...
'If you check "Overwrite Existing", information about a subscriber in the database will be replaced by the imported information. Subscribers are matched by email or foreign key.' => 'Hvis du v&aelig;lger "Overskriv eksisterende", vil information om en bruger i databasen blive erstattet med den importerede information. Brugere matches efter email eller fremmedn&oslash;gle.',
'If you check "Retain Old Email", a conflict of two emails being the same will keep the old one and add "duplicate" to the new one. If you don&quot;t check it, the old one will get "duplicate" and the new one will take precedence.' => 'Hvis du v&aelig;lger "Bevar gammel bruger email", vil en konflikt over at de to emails er identiske bevare den gamle og tilf&oslash;je "dobbelt forekomst" til den nye. Hvis du ikke v&aelig;lger, vil den gamle f&aring; "dobbelt forekomst" og den nye vil have f&oslash;rsteret.',
'If you choose "send notification email" the subscribers you are adding will be sent the request for confirmation of subscription to which they will have to reply. This is recommended, because it will identify invalid emails.' => 'Hvis du v&aelig;lger "send besked email" vil brugerne du tilf&oslash;jer f&aring; tilsendt en bekr&aelig;ftelse p&aring; tilmeldingen, som de skal besvare. Det kan anbefales, idet det vil afsl&oslash;re ugyldige emails.',
