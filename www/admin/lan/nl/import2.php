...
'automatically added' => 'automatisch toegevoegd',
 'The file you upload will need to have the attributes of the records on    the first line.     Make sure that the email column is called "email" and not something like "e-mail" or     "Email Address".     Case is not important.          If you have a column called "Foreign Key", this will be used for synchronisation between an     external database and the phpList database. The foreignkey will take precedence when matching     an existing subscriber. This will slow down the import process. If you use this, it is allowed to have     records without email, but an "Invalid Email" will be created instead. You can then do     a search on "invalid email" to find those records. Maximum size of a foreign key is 100.          Warning: the file needs to be plain text. Do not upload binary files like a Word Document.     ' => '<p>Het bestand dat he het geupload zal het attribuut van de verslagen op de eerste lijn nodig hebben.
    ...
    <br/>',
'The following limits are set by your server: Maximum size of a total data sent to server: %s Maximum size of each individual file: %s' => 'De volgende limieten zijn ingesteld door jou server:<br/>
...
'If you check "Test Output", you will get the list of parsed emails on screen, and the database will not be filled with the information. This is useful to find out whether the format of your file is correct. It will only show the first 50 records.' => 'Als u "Test Uitvoer" aanvinkt, dan krijgt u een lijst met geparste emails op uw scherm, en de database zal niet worden gevuld met de data. Dit is nuttig om uit te zoeken of het formaat van je bestand correct is. Het zal enkel de eerste 50 records tonen.',
'If you check "Show Warnings", you will get warnings for invalid records. Warnings will only be shown if you check "Test Output". They will be ignored when actually importing. ' => 'Als u "Toon Waarschuwingen" aanvinkt, dan krijgt u waarschuwingen voor ongeldige records. Waarschuwingen zullen enkel worden getoond als "Test Output" is aangevinkt. Ze zullen worden genegeerd als je echt aan het importeren bent. ',
'If you check "Omit Invalid", invalid records will not be added. Invalid records are records without an email. Any other attributes will be added automatically, ie if the country of a record is not found, it will be added to the list of countries.' => 'Als u "Laat Ongeldig Weg" aanvinkt, dan zullen ongeldige records niet worden toegevoegd. Ongeldige records zijn records zonder email. Alle andere attribute zullen automatisch worden toegevoegd, bv als het land van een record niet is gevonden, dan zal het worden toegevoegd aan de lijst met landen.',
'Assign Invalid will be used to create an email for subscribers with an invalid email address. You can use values between [ and ] to make up a value for the email. For example if your import file contains a column "First Name" and one called "Last Name", you can use "[first name] [last name]" to construct a new value for the email for this subscriber containing their first name and last name. The value [number] can be used to insert the sequence number for importing.' => 'Wijs Ongeldig Toe zal worden gebruikt om een email te creeren voor gebruikers met een ongeldig email adres.
...
'If you check "Overwrite Existing", information about a subscriber in the database will be replaced by the imported information. Subscribers are matched by email or foreign key.' => 'Als je "Overschrijf Bestaand" aanvinkt, dan zal info over een gebruiker in de database worden vervangen door de geimporteerde info. Gebruikers komen overeen door email of foreign key.',
'If you check "Retain Old Email", a conflict of two emails being the same will keep the old one and add "duplicate" to the new one. If you don&quot;t check it, the old one will get "duplicate" and the new one will take precedence.' => 'Als je "Behoud Oude Gebruiker Email" aanvinkt, dan zal een conflict tussen twee emails die hetzelfde zijn de oude worden bewaard en "duplicaat" toevoegen aan de nieuwe. Als je dit niet aanvinkt, dan zal de oude "duplicaat" krijgen en de nieuwe zal voorrang krijgen.',
'If you choose "send notification email" the subscribers you are adding will be sent the request for confirmation of subscription to which they will have to reply. This is recommended, because it will identify invalid emails.' => 'Als je "verzend notificatie email" dan zullen de gebruikers die je toevoegd een verzoek ter bevestiging ontvangen waarop ze zullen moeten antwoorden. Dit is aangeraden omdat het ongeldige emails identificeerd.',
