...
'automatically added' => 'automatikusan hozzáadva',
 'The file you upload will need to have the attributes of the records on    the first line.     Make sure that the email column is called "email" and not something like "e-mail" or     "Email Address".     Case is not important.          If you have a column called "Foreign Key", this will be used for synchronisation between an     external database and the phpList database. The foreignkey will take precedence when matching     an existing subscriber. This will slow down the import process. If you use this, it is allowed to have     records without email, but an "Invalid Email" will be created instead. You can then do     a search on "invalid email" to find those records. Maximum size of a foreign key is 100.          Warning: the file needs to be plain text. Do not upload binary files like a Word Document.     ' => '<p class="information">Az Ön által feltöltendő fájl első sorában kell lennie a rekordok tulajdonságainak.
    ...
    <br/>',
'The following limits are set by your server: Maximum size of a total data sent to server: %s Maximum size of each individual file: %s' => 'A kiszolgáló a következő korlátozásokat állította be:<br/>
...
'If you check "Test Output", you will get the list of parsed emails on screen, and the database will not be filled with the information. This is useful to find out whether the format of your file is correct. It will only show the first 50 records.' => 'Ha bejelöli a "Kimenet tesztelése" beállítást, akkor az elemzett e-mail címek meg fognak jelenni a képernyőn, s nem fogja megtölteni az adatbázist az adatokkal. Ez a fájl megfelelő formátumának megállapításakor hasznos. Csak az első 50 rekord lesz látható.',
'If you check "Show Warnings", you will get warnings for invalid records. Warnings will only be shown if you check "Test Output". They will be ignored when actually importing. ' => 'Ha bejelöli a "Láthatók a figyelmeztetések" beállítást, akkor meg fognak jelenni az érvénytelen rekordok figyelmeztetései. A figyelmeztetések csak akkor lesnek láthatók, ha bejelöli a "Kimenet tesztelése" beállítást. A tényleges importáláskor figyelmen kívül fogja őket hagyni. ',
'If you check "Omit Invalid", invalid records will not be added. Invalid records are records without an email. Any other attributes will be added automatically, ie if the country of a record is not found, it will be added to the list of countries.' => 'Az "Érvénytelenek kihagyása" beállítás bejelölésekor nem adja hozzá az érvénytelen rekordokat. Az érvénytelen rekordok e-mail cím nélküli rekordok. Bármilyen más tulajdonság automatikusan hozzáadásra kerül, ha például nem található egy rekord országa, akkor fel fogja venni az országlistára.',
'Assign Invalid will be used to create an email for subscribers with an invalid email address. You can use values between [ and ] to make up a value for the email. For example if your import file contains a column "First Name" and one called "Last Name", you can use "[first name] [last name]" to construct a new value for the email for this subscriber containing their first name and last name. The value [number] can be used to insert the sequence number for importing.' => 'Az "Érvénytelenek hozzárendelése" beállítás az érvénytelen e-mail címmel rendelkező felhasználók számára küldendő e-mail készítéséhez kerül felhasználásra.
...
'If you check "Overwrite Existing", information about a subscriber in the database will be replaced by the imported information. Subscribers are matched by email or foreign key.' => 'A "Meglévők felülírása" beállítás bejelölésekor a felhasználónak az adatbázisban tárolt adatait ki fogja cserélni az importált információkkal. A felhasználók egyeztetése e-mail cím vagy idegen kulcs alapján történik.',
'If you check "Retain Old Email", a conflict of two emails being the same will keep the old one and add "duplicate" to the new one. If you don&quot;t check it, the old one will get "duplicate" and the new one will take precedence.' => '"A felhasználó régi e-mail címének megőrzése" beállítás bejelölésekor, ha a két e-mail cím azonos, akkor a régit tartja meg, s az újhoz hozzáfűzi a "duplum" jelzést. Ha nem jelöli be ezt a beállítást, akkor a régi kapja a "duplum" megjelölést, s az újat részesíti előnyben.',
'If you choose "send notification email" the subscribers you are adding will be sent the request for confirmation of subscription to which they will have to reply. This is recommended, because it will identify invalid emails.' => 'Az "Értesítő e-mail küldése" beállítás választásakor az Ön által hozzáadott felhasználóknak elküldi a feliratkozás visszaigazolásának kérését, melyre válaszolniuk kell. Ez ajánlott, mert beazonosítja az érvénytelen e-mail címeket.',
'phplist Import  Results' => 'phplist importálási eredmény',
