...
'automatically added' => '自動的に追加されました。',
 'The file you upload will need to have the attributes of the records on    the first line.     Make sure that the email column is called "email" and not something like "e-mail" or     "Email Address".     Case is not important.          If you have a column called "Foreign Key", this will be used for synchronisation between an     external database and the phpList database. The foreignkey will take precedence when matching     an existing subscriber. This will slow down the import process. If you use this, it is allowed to have     records without email, but an "Invalid Email" will be created instead. You can then do     a search on "invalid email" to find those records. Maximum size of a foreign key is 100.          Warning: the file needs to be plain text. Do not upload binary files like a Word Document.     ' => '<p>アップロードするファイルの最初の行にレコードの属性が記載されている必要があります。
    ...
    <br/>',
'The following limits are set by your server: Maximum size of a total data sent to server: %s Maximum size of each individual file: %s' => '以下の制限があなたのウェブサーバによって設定されています:<br/>
サーバに送るトータルデータの最小サイズ: <b>%s</b><br/>
ここのファイルについての最大サイズ: <b>%s</b>
<br/>PHPlistは1Mbより大きなファイルは処理しません。',
'If you check "Test Output", you will get the list of parsed emails on screen, and the database will not be filled with the information. This is useful to find out whether the format of your file is correct. It will only show the first 50 records.' => 'もし"テスト出力"をチェックするのでしたら、画面上に分析された電子メールのリストが表示されます。このときデータベースには情報が保存されません。これはファイルが正しいかどうかをみつけることに役立ちます。最初の50レコードのみを表示します。',
'If you check "Show Warnings", you will get warnings for invalid records. Warnings will only be shown if you check "Test Output". They will be ignored when actually importing. ' => '"警告を表示"をチェックするのであれば、妥当でないレコードの警告を得ることができます。”テスト出力"をチェックすると警告が表示されるだけです。実際にインポート中のときそれらは無視されます。',
'If you check "Omit Invalid", invalid records will not be added. Invalid records are records without an email. Any other attributes will be added automatically, ie if the country of a record is not found, it will be added to the list of countries.' => '"Invalid 省略"をチェックすると、妥当でないレコードは追加されません。妥当でないレコードとは電子メールを持たないレコードです。他の属性は自動的に追加されます。すなわち、もし１レコードについてcountryが見つからないとき、countriesのリストに追加されます。',
'Assign Invalid will be used to create an email for subscribers with an invalid email address. You can use values between [ and ] to make up a value for the email. For example if your import file contains a column "First Name" and one called "Last Name", you can use "[first name] [last name]" to construct a new value for the email for this subscriber containing their first name and last name. The value [number] can be used to insert the sequence number for importing.' => '"Invalid 割り当て"は、Invalid Emailをもつユーザに電子メールを作成することに使用されます。
...
'If you check "Overwrite Existing", information about a subscriber in the database will be replaced by the imported information. Subscribers are matched by email or foreign key.' => 'もしあなたが"既存を上書き"をチェックするのでしたら、データベース内のユーザの情報はインポートされた情報で置換されます。ユーザは電子メールアドレスまたは外部キーによりマッチされます。',
'If you check "Retain Old Email", a conflict of two emails being the same will keep the old one and add "duplicate" to the new one. If you don&quot;t check it, the old one will get "duplicate" and the new one will take precedence.' => 'もし"古いユーザ電子メールを保持"をチェックするのでしたら、同一である二つの電子メールアドレスのコンフリクトは、古い電子メールアドレスをそのまま維持し、新しい電子メールアドレスに"duplicate"を追加します。もしチェックしないから、古い電子メールアドレスは"duplicate"となり、新しい電子メールアドレスが優先されるでしょう。',
