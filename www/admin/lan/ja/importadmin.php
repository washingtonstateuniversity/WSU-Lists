...
$lan = array(
 ...
 <b>警告</b>: ファイルはプレーンテキストである必要があります。Wordドキュメントのようなバイナリファイルをアップロードしないでください。
  ...
  'If you check "Test Output", you will get the list of parsed emails on screen, and the database will not be filled with the information. This is useful to find out whether the format of your file is correct. It will only show the first 50 records.' => 'もし、"テスト出力"をチェックするのであれば、あなたの画面上に解析された電子メールのリストが表示されますが、データベースには情報は保存されないでしょう。これはあなたのファイルのフォーマットが正しいかどうかみつけるのに役立ちます。最初の50レコードのみ表示されるでしょう。',
  ...
  'Test output:There should only be ONE email per line.If the output looks ok, go Back to resubmit for real' => 'テスト出力:<br>1行につき1つの電子メールのみであるべきです。<br>もし出力がOKでしたら、本当に再送信するためには<a href="javascript:history.go(-1)">戻る</a>に行ってください。<br><br>',  
  'List for' => 'List for',
'login' => 'ログイン',
