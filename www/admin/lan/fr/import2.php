...
'automatically added' => 'ajout&eacute;s automatiquement',
 'The file you upload will need to have the attributes of the records on    the first line.     Make sure that the email column is called "email" and not something like "e-mail" or     "Email Address".     Case is not important.          If you have a column called "Foreign Key", this will be used for synchronisation between an     external database and the phpList database. The foreignkey will take precedence when matching     an existing subscriber. This will slow down the import process. If you use this, it is allowed to have     records without email, but an "Invalid Email" will be created instead. You can then do     a search on "invalid email" to find those records. Maximum size of a foreign key is 100.          Warning: the file needs to be plain text. Do not upload binary files like a Word Document.     ' => '<p class="information">Le fichier que vous t&eacute;l&eacute;chargez devra comporter les attributs des donn&eacute;es sur la premi&egrave;re ligne.
    ...
    <br/>',
'The following limits are set by your server: Maximum size of a total data sent to server: %s Maximum size of each individual file: %s' => 'Les limites suivantes sont d&eacute;finies par votre serveur:<br/>
...
'If you check "Overwrite Existing", information about a subscriber in the database will be replaced by the imported information. Subscribers are matched by email or foreign key.' => 'Si vous cochez "Ecraser valeurs existantes", les informations sur un utilisateur dans la base de donn&eacute;es seront remplac&eacute;es par l&rsquo;information import&eacute;e.  Les utilisateurs sont li&eacute;es par email ou cl&eacute; &eacute;trang&egrave;re (foreign key).',
'If you check "Retain Old Email", a conflict of two emails being the same will keep the old one and add "duplicate" to the new one. If you don&quot;t check it, the old one will get "duplicate" and the new one will take precedence.' => 'Si vous cochez "Garder ancien email utilisateur", s&rsquo;il existe une conflit entre deux emails identiques, le plus ancien sera retenu et le nouveau sera marqu&eacute; "double".  Si vous ne cochez pas cette option, l&rsquo;ancien sera marqu&eacute; "double" et le nouveau prendra le dessus.',
'If you choose "send notification email" the subscribers you are adding will be sent the request for confirmation of subscription to which they will have to reply. This is recommended, because it will identify invalid emails.' => 'Si vous cochez "envoyer email de notification" les utilisateurs que vous ajoutez vont recevoir une demande de confirmation &agrave; la liste, demande &agrave; laquelle il leur faudra r&eacute;pondre. Ceci est recommand&eacute;, car cela vous permettra d&rsquo;identifier des emails non valides',
